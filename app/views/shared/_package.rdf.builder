...
xml.rdf(:Description, 'rdf:about'=>"#{work_path(work)}") do
  ...
  if work.people.present?
    xml.core(:informationResourceInAuthorship, {'rdf:resource' => "#{work_path(work)}#Authorship"})
  end
end

xml.rdf(:Description, {'rdf:about'=>"#{work_path(work)}#Authorship"}) do
  xml.rdf(:type, {'rdf:resource'=>"http://vivoweb.org/ontology/core#Authorship"})
  xml.core(:linkedInformationResource, {'rdf:resource'=> work_path(work)})
