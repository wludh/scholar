...
class UsersController < ApplicationController
  ...
  def create
    ...
    if @user.save
      ...
      redirect_to "/scholar/works" + root_path
    else
      render :action => 'new'
    end
  end
  ...
  def update
    ...
    if @user.save
      redirect_back_or_default("/scholar" + root_path(:locale => @user.default_locale))
    else
      render :action => 'edit'
    end
  end
  ...
  def request_update_email
    ...
    if new_email == @user.email
      ...
      redirect_to("/scholar" + edit_user_path(@user)) and return
    end
    ...
    if !@user.valid?
      ...
      redirect_back_or_default("/scholar" + root_path)
    end
  ...
  def update_email
    ...
    if code == user.email_update_code(new_email)
      ...
      if user.save
        ...
        redirect_to("/scholar" + edit_user_path(user))
      end
    else
  ...
  def activate
    ...
    if user
      redirect_to "/scholar" + login_path(:locale => user.default_locale) + "&return_to=%2F"
    else
      redirect_to "/scholar" + root_path
    end
  end
