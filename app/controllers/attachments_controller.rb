...
class AttachmentsController < ApplicationController
  ...
  def create
    ...
    if params[:file].nil?
      respond_to do |format|
        ...
        format.html { redirect_to "/scholar" + new_person_attachment_path(@asset.id) }
    ...
    respond_to do |format|
      if @asset.save
        ...
        if @asset.kind_of?(Person)
          format.html { redirect_to "/scholar" + edit_person_attachment_path(@asset.id, @asset.image.id) }
    ...
    if params[:file].blank?
      respond_to do |format|
        ...
        if @attachment.asset.kind_of?(Person)
          format.html { redirect_to "/scholar" + edit_person_attachment_path(@attachment.asset.id, @attachment.id) }
          format.xml { head :ok }
        else
  ...
  def get_response_url(asset)
    if asset.kind_of?(Work)
      #return to Work page
      return "/scholar" + work_path(asset)
    elsif asset.kind_of?(Person)
      #return to Person page
      return "/scholar" + person_path(asset)
    end
  end
