class PublicationsController < ApplicationController
    ...
    response_for :update do |format|
      format.html do
        if params[:save_and_list]
          redirect_to "/scholar" + publications_path(:page => @publication.name.first.upcase)
        else
          redirect_to "/scholar" + publication_path(@publication)
        end
      end
      format.rss
    end
  ...
  def destroy
    ...
    return_path = params[:return_path] || publications_path
    ...
    respond_to do |format|
      if full_success
        ...
        format.html { redirect_to "/scholar" + edit_publication_path(publication.id) }
        format.xml { head :ok }
      end
    end
  end
