...
module PubCommonHelper
  ...
  def update_multiple_generic(klass)
    with_key_and_path_setup(klass) do |key, path|
      ...
      respond_to do |wants|
        wants.html do
          redirect_to("/scholar" + self.send(path, :page => @page))
        end
      end
    end
  end
  ...
  def operate_on_box_generic(klass)
    with_key_and_path_setup(klass) do |key, path|
      ...
      begin
        ...
        redirect_to("/scholar" + self.send(path)) and return
      end
      session[key] = @pas
      redirect_to("/scholar" + self.send(path, :page => @page)) unless request.xhr?
    end
  end
