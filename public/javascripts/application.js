...
function confirm_delete_for_items_selected_by(checkbox_selector) {
  ...
  var msg = $jq.t("Are you sure you want to delete these duplicates?", {count:count});
  return confirm(msg);
}
...
function validateWLUEmail(email) {
    var re = /^\s*[\w\-\+_]+(\.[\w\-\+_]+)*\@[\w\-\+_]+\.[\w\-\+_]+(\.[\w\-\+_]+)*\s*$/;
    if (re.test(email)) {
        if (email.indexOf('@wlu.edu', email.length - '@wlu.edu'.length) !== -1) {
            return true;
        } else {
	    return false;
        }
